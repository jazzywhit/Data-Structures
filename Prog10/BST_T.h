...
class BST
{
	...
	private:
		...
		// Recursive remove the leftmost leave node and return its data.
...
PRECONDITION
the subtree "r" is not empty.
INPUT PARAMETERS
r - the root node of the subtree from which the leaf node
is to be removed
RETURN VALUE
the data part of the removed leaf node
*/
template <typename NodeData>
NodeData BST<NodeData>::RRemoveLeaf(Node *&r)
{
	...
	if (r->left != 0) // There is a left successor, traverse left subtree
		return RRemoveLeaf(r->left);
	else if (r->right != 0) // There is a right successor, traverse right subtree

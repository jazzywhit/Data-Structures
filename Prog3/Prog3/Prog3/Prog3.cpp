/*--------------- P r o g 3 . c p p ---------------
 ...
 CHANGES
 09-24-2011 gpc -  Created for 16.322                 
 */
...
string GetCmd(istream &cmdStream, LinkedList &polygon)
{
    ...
    if (!polygon.AtEnd() && cmdStream == cin)
    {
        ...
        NodeData data = polygon.CurrentEntry();
        cout << data;
...
{
	polygon.Rewind();
	Point firstPoint = polygon.CurrentEntry(); //Does not change, as the first point is the origin for all other triangles.
    Point currPoint;
    Point nextPoint;
